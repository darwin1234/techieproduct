...
class CenposConnector extends ModelConnector {
    ...
    public function TokenFView($Request, $Url = "") {
        ...
        $Response["IsJquery"] = $Request->Jquery;
    ...
    public function AddCryptoToken($Request, $Url = "") {
        ...
        if (empty($Url))
            $Url = "https://ww3.cenpos.net/2/ecommerceTokens.asmx?wsdl";
    ...
    public function ValidateToken($Request, $Url = "") {
        if (empty($Url))
            $Url = "https://ww3.cenpos.net/6/4/ecommerce.asmx?wsdl";
    ...
    public function ConvertToPermanentToken($Request, $Url = "") {
        ...
        $ResponseCard = $this->CallFunctionClient($client, "AddCardToken", $Request);
        
        if($ResponseCard->AddCardTokenResult->Result == 606) $ResponseCard->AddCardTokenResult->Result = 0;
    ...
    public function VoidBView($Request, $Url = "") {
      ...
      //  $Response["Password"] = base64_encode($Request->Password);
       // $Response["Userid"] = $Request->UserId;
        ...
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt ($ch, CURLOPT_POST, 1);
        curl_setopt ($ch, CURLOPT_POSTFIELDS, "merchant=$merchant&password=$password&ip=$ip&userid=$userid&referencenumber=$ReferenceNumber");
        curl_setopt($ch,CURLOPT_RETURNTRANSFER, true);
        $resconn = curl_exec($ch);
        $error = curl_error($ch);
        curl_close ($ch);
        ...
        $ResponseEncrypted = json_decode($resconn);
        if($ResponseEncrypted->Result != 0) { die($ResponseEncrypted->Message); }
        $Response["verifyingpost"] = urlencode($ResponseEncrypted->Data);
